...
// Helper: extract plain hostname from full URL
...
const WAITING_TIMEOUT = 180000;
const MODEL_RESTART_TOAST_DELAY = 2000;
const MAX_IMAGES = 5;
...
function loadSettings() {
  chrome.storage.local.get(['picoclawSettings', 'picoclawSessionId', 'inputText', 'picoclawWaiting', 'picoclawAttachedImages'], (result) => {
    ...
    const welcomeMsg = document.querySelector('#chatMessages .bot-message .message-content p');
    if (welcomeMsg) {
...
async function login() {
  try {
    ...
    const actualHost = extractHost(host);
              ...
              chrome.runtime.sendMessage({ type: 'connect' }, () => {
                ...
                const checkConnect = setInterval(() => {
                  ...
                  chrome.runtime.sendMessage({ type: 'get_status' }, (sr) => {
                    ...
                    } else if (attempts >= CONNECTION_MAX_ATTEMPTS) {
                      clearInterval(checkConnect);
...
function handleImageSelect(e) {
  ...
  if (attachedImages.length + files.length > MAX_IMAGES) {
    showToast(`最多只能上傳 ${MAX_IMAGES} 張圖片`);
    imageInput.value = '';
    return;
  }
  ...
  const processFile = (file, index) => {
    ...
    if (file.size > IMAGE_MAX_SIZE) {
      showToast(`圖片 ${file.name} 超過 5MB 限制`);
      return;
    }
...
function getApiBaseUrl() {
  ...
  const host = extractHost(settings.serverUrl);
...
async function setDefaultModel(modelName) {
  try {
    ...
    setTimeout(() => showToast(`已切换模型: ${modelName}`), MODEL_RESTART_TOAST_DELAY);
  } catch (e) {
    showToast('切换失败');
  }
}
...
function deleteSession(sessionId, title) {
  if (!confirm(t('deleteConfirm1', { title }))) return;
  if (!confirm(t('deleteConfirm2', { title }))) return;
